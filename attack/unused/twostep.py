...
import time
import unused.zvp_glv_ltr as zvp_glv_ltr, zvp_glv_sac, zvp_glv_binary
...
def twostep_sm(params, verbose=True):
    ...
    result = zvp_glv_ltr.zvp_glv_sm(params, verbose)
    ...
    assert P != curve(0)
    Q = zvp_glv_ltr.ltr_oracle(curve, k, P, 0)[0]
...
def twostep_sm_set(params, verbose=True):
    ...
    curve, lam, order = params["curve"], params["lam"], params["order"]
    k, k0, k1 = params["k"], params["k0"], params["k1"]
    result = zvp_glv_ltr.zvp_glv_sm_set(params, verbose)
    assert k == (k0 + k1 * lam) % order
    # Baby step giant step on lower bits
    P = curve.random_point()
    assert P != curve(0)
...
def twostep_msm(params, verbose=True):
    ...
    k, k0, k1 = params["k"], params["k0"], params["k1"]
    result = zvp_glv_binary.zvp_glv_msm_shamir(params, verbose)
    ...
    P = curve.random_point()
    assert P != curve(0)
    Q = msm.shamir_scalar_mul_oracle_positive(k0, k1, P, lam, beta, order, 0)[0]
    assert Q == k0 * P + k1 * lam * P
    time_bsgs = time.time()
...
def twostep_msm_shamir(params, verbose=True):
    """2-step attack on Shamir's MSM"""
    curve, lam, order, beta = (
        params["curve"],
        params["lam"],
        params["order"],
